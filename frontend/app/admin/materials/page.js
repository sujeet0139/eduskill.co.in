        ...
        <tbody className="divide-y">
          {filtered.length === 0 ? (
            ...
            <tr key={m.id} className="hover:bg-gray-50">
              <Td className="font-medium">{m.title}<div className="text-xs text-gray-400">{m.category}</div></Td>
              <Td className="text-xs">{tag(m)}</Td>
              <Td className="text-xs">{m.subject || "—"}</Td>
              <Td><a href={api.mediaUrl(m.file_path)} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">View</a></Td>
